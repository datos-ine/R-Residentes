...
Supongamos que un labor habitual en nuestro trabajo sea convertir en años la diferencia entre dos fechas, por ejemplo entre la fecha de nacimiento y otra, para calcular la edad.
...
```{r}
igini2 <- function(casos, poblacion){
  ...
  h1 <- h.select(df$apob, df$acasos, method = 'cv')
  giniorig <- sm.regression(df$apob, df$acasos, h = h1, display = "none")      # <3>
  AUCorig <- trapz(giniorig$eval.points, giniorig$estimate)
  Giniorig1 <- abs((AUCorig-0.5)*2)
...
2. Usamos formato tidyverse para las cosas que se pueden hacer de esta forma, aplicando el abrazo de las variables de argumento.
3. Agregamos argumento *display = "none"* para anular el plot innecesario de la función `sm.regression()`
4. Incorporamos el formateo del valor del índice Gini calculado dentro de la función
...
O si necesitamos asignar el valor en un vector independiente:
...
Una de propiedades más interesantes de las funciones creadas es que las podemos introducir en un script que llamaremos en nuestro trabajo cada vez que necesitemos ejecutarlas.

La función `source()` nos permite llamar a ese o esos scripts externos y contar con las funciones a nuestra disposición.

Entonces podemos automatizar algunas tareas, sobre todo en procesos repetitivos o que tienen un mismo esquema. Un ejemplo concreto en la epidemiología son las tareas vinculadas al análisis de datos provenientes de la vigilancia epidemiológica.

La forma de hacerlo es sencilla, si mi scripts de funciones se llama "funciones.R" ejecutamos:
